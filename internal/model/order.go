...
type Order struct {
	...
	// 添加普通索引：用户查询自己的订单列表是非常高频的操作
	UserID uint `json:"user_id" gorm:"not null;default:1;index:idx_user_id"`

	// 添加唯一索引：系统内部生成的商户单号，必须唯一，且支付回调时常用来做精准匹配
	Order_id string `json:"order_id" gorm:"size:255;uniqueIndex:uk_order_id"`

	Type         int   `json:"type" gorm:"not null"` // 1=微信, 2=支付宝
	Price        int64 `json:"price" gorm:"not null"`
	Really_price int64 `json:"really_price" gorm:"not null"`

	// 添加普通索引：常用来统计成功单数、筛选订单状态
	State int `json:"state" gorm:"not null;index:idx_state"` // -1=已关闭, 0=待支付, 1=已支付, 2=通知失败

	Notify_url string `json:"notify_url" gorm:"size:255"`
	Return_url string `json:"return_url" gorm:"size:255"`

	// 添加普通索引：第三方支付平台的流水号，方便根据上游流水号排查账单
	Pay_id string `json:"pay_id" gorm:"size:255;index:idx_pay_id"`

	Pay_url string `json:"pay_url" gorm:"size:255"`
	Param   string `json:"param" gorm:"size:255"`
	Is_auto int    `json:"is_auto" gorm:"not null"`

	// ⭐️ 核心优化：局部索引 (Partial Index)
	// 只有当 state = 0 时，这条记录的 expire_at 才会被加入索引树。
	// 这将完美解决你之前那个耗时 200ms 的更新语句。
	ExpireAt int64 `json:"expire_at" gorm:"not null;index:idx_pending_expire,where:state = 0"`
...
// IsExpired 检查订单是否过期
func (o *Order) IsExpired(remainingSeconds int64) bool {
...
type OrderResponse struct {
	...
	Really_price int64  `json:"really_price"`
	State        int    `json:"state"`
	State_text   string `json:"state_text"`
