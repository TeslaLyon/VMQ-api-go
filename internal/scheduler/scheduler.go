...
func NewScheduler(orderService service.OrderService, monitorAndroidService service.MonitorAndroidService) *Scheduler {
	return &Scheduler{
...
func (s *Scheduler) closeExpiredOrders() {
	...
	closedCount, err := s.orderService.CloseExpiredOrdersByExpiredAt()
	if err != nil {
...
func (s *Scheduler) checkMonitorStatus() {
	...
	// 获取所有用户的监控状态并检查心跳超时
	err := s.monitorAndroidService.CheckAndUpdateMonitorStatus()
	if err != nil {
