...
type OrderService interface {
	...
	CloseExpiredOrdersByExpiredAt() (int64, error) // 新增：根据过期时间关闭订单
}
...
func (s *orderService) GetOrderStatus(orderID string) (*model.OrderStatusResponse, error) {
	...
	remainingSeconds := int64(order.ExpireAt - time.Now().Unix())
	orderState := order.State
	if remainingSeconds <= 10 {
		// 剩余时间不多了就关闭订单，防止用户一直不支付
		orderState = -1
		remainingSeconds = 0
	...
	return &model.OrderStatusResponse{
		...
		Is_expired: order.IsExpired(remainingSeconds),
...
func (s *orderService) generateExpireAt(userID uint) int64 {
	expireMinutes := s.getOrderExpireMinutes(userID)
	return time.Now().Add(time.Duration(expireMinutes) * time.Minute).Unix()
}

func (s *orderService) generateReallyPrice(price int64, payType int, oid string) int64 {
	return s.orderRepo.GenerateUniquePrice(price, payType, oid)
}
...
func (s *orderService) closeExpiredOrders() (int64, error) {

	return 0, nil
}

func (s *orderService) closeExpiredOrdersByExpiredAt() (int64, error) {
	return s.orderRepo.CloseExpiredOrdersForCronJob()
}
...
func (s *orderService) CloseExpiredOrdersByExpiredAt() (int64, error) {
	return s.closeExpiredOrdersByExpiredAt()
}
...
func (s *orderService) GenerateReturnURL(orderID string) (string, error) {
	...
	reallyPriceStr := fmt.Sprintf("%d", order.Really_price)
...
func (s *orderService) generatePayURL(userID uint, orderType int, price int64) string {
	// 1. 优先查找与金额完全匹配的收款码
	...
	return fmt.Sprintf("https://test-payment.vmqfox.com/%s?amount=%d", paymentMethod, price)
}
