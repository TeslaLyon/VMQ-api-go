...
func (h *QrcodeHandler) UpdateQrcodeStatus(c *gin.Context) {
	...
	if qrcode == nil {
		response.InternalError(c, "Qrcode not found")
		return
	}
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		response.BadRequest(c, fmt.Sprintf("Failed to bind JSON: %s", err.Error()))
		return
	}
...
func (h *QrcodeHandler) checkQrcodeAccess(c *gin.Context, qrcodeID uint) (*model.PayQrcode, bool) {
	...
	if !middleware.ValidateResourceAccess(c, qrcode.User_id) {
		response.InternalError(c, "Access denied: insufficient permissions")
		return nil, false
	}
