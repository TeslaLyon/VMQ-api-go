...
func NewOrderHandler(orderService service.OrderService, monitorAndroidService service.MonitorAndroidService, userService service.UserService) *OrderHandler {
	return &OrderHandler{
...
func (h *OrderHandler) checkOrderAccess(c *gin.Context, orderID uint) (*model.Order, bool) {
	...
	// 检查数据访问权限
	if !middleware.ValidateResourceAccess(c, order.UserID) {
...
func (h *OrderHandler) GetOrderUnified(c *gin.Context) {
	...
	// fmt.Printf("Accessing GetOrderUnified with order_id=%s, access_type=%s\n", orderID, accessType)
...
func (h *OrderHandler) getOrderForPayment(c *gin.Context, orderID string) {
	...
	// closeTime := 5 // 默认5分钟 2026年05月31日20:44:12 注释，因为不传递 jwt 无法读取各个账户设置的过期时间
	...
	remainingSeconds := int(order.ExpireAt - time.Now().Unix())
	orderState := order.State
	if remainingSeconds <= 10 {
		// 剩余时间不多了就关闭订单，防止用户一直不支付
		orderState = 0
		remainingSeconds = 0
		h.orderService.CloseOrder(order.Id)
		log.Printf("订单已关闭：订单查询接口时检测到剩余时间不足 10 秒，订单ID：%s", order.Order_id)
	}
	// 转换为支付页面格式的响应
	paymentOrder := &model.PaymentOrderResponse{
		...
		Is_expired:       remainingSeconds <= 0 && orderState == 0,
		Expired_at:       order.ExpireAt,
